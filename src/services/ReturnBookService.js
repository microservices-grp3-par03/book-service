...
async function consumeMessagesFromReturnBookQueue() {
  try {
    const channel = await connectRabbitMQ();

    const queue = 'cancel_borrowing_queue';
      ...
      async (msg) => {
        if (msg !== null) {
          try {
            ...
            const { bookId, borrowingId, isAvailable } = data;
            ...
            console.log(`Traitement de l'annulation pour le livre ${bookId}`);
            ...
            if (result) {
              console.log(`Livre ${bookId} marqué comme disponible.`);
            } else {
              console.error(
...
module.exports = { consumeMessagesFromReturnBookQueue };
